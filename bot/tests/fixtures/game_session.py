...
async def game_session_1(
    ...
    game_session = GameSessionModel(chat_id=chat_1.id, creator_id=creator_1.id, state=0, )
    async with db_session.begin() as session:
...
async def game_session_2(
    ...
    game_session = GameSessionModel(chat_id=chat_2.id, creator_id=creator_2.id, state=0, )
    async with db_session.begin() as session:
        session.add(game_session)
