...
class TestHandleBaseUpdates:
    ...
    async def test_handle_participate(self, store, preparing_state, participate_update_player_1):
    ...
    async def test_handle_launch(self, store, preparing_state, participate_update_player_1, launch_game_update):
        """"""
        await store.bots_manager.handle_update(update=participate_update_player_1)
        await store.bots_manager.handle_update(update=launch_game_update)
        assert store.external_api.send_message.call_count == 4

    async def test_handle_right_word(self, store, db_session, chat_1, player_1, creator_1, waiting_word_state, word_update_player_1, word_update_creator_1):
        ...
        async with db_session.begin() as session:
            ...
            prev_session_player = await store.game_sessions.get_session_player(session, prev_player_id, 1)
        ...
        async with db_session.begin() as session:
            game_session = await store.game_sessions.get_current_session(session, chat_1.id)
        assert game_session.state == StatesEnum.VOTE.value

    async def test_handle_wrong_word(self, store, db_session, chat_1, player_1, creator_1, waiting_word_state, wrong_word_update_player_1, wrong_word_update_creator_1):
    ...
    async def test_handle_two_full_turns(self,
                                         store,
                                         db_session,
                                         chat_1,
                                         player_1,
    ...
    async def test_handle_one_yes_vote(self, store, db_session, chat_1, player_1, creator_1, waiting_votes_state, creator_vote_yes_update, player_vote_yes_update):
    ...
    async def test_handle_one_no_vote(self, store, db_session, chat_1, player_1, creator_1, waiting_votes_state, player_vote_no_update, creator_vote_no_update):
        ...
        async with db_session.begin() as session:
            game_session = await store.game_sessions.get_current_session(session, chat_1.id, StatesEnum.ENDED.value)
        assert game_session

    async def test_handle_repeating_yes_vote(self, store, db_session, chat_1, player_1, player_2, creator_1, waiting_votes_state_three_players, creator_vote_yes_update, player_vote_yes_update):
    ...
    async def test_word_timeout(self, db_session, server, chat_1, store, waiting_word_state):
        ...
        async with db_session.begin() as session:
            game_session = await store.game_sessions.get_current_session(session, chat_1.id, StatesEnum.ENDED.value)
            players_in_session = await store.game_sessions.get_session_players(session, 1)
        assert game_session
    ...
    async def test_vote_timeout(self, db_session, server, chat_1, store, waiting_votes_state):
    ...
    async def test_handle_end_update(self, db_session, server, chat_1, store, waiting_word_state, end_game_update):
    ...
    async def test_handle_end_update_2(self, db_session, server, chat_1, store, waiting_votes_state_three_players, end_game_update):
    ...
    async def test_handle_end_update_3(self, db_session, server, chat_1, store, preparing_state, end_game_update):
    ...
    async def test_handle_end_wrong_update(self, db_session, server, chat_1, store, waiting_votes_state_three_players, wrong_end_game_update):
        ...
        assert store.external_api.send_message.call_count == 7
        async with db_session.begin() as session:
            game_session = await store.game_sessions.get_current_session(session, chat_1.id, StatesEnum.VOTE.value)
        assert game_session

    async def test_handle_end_wrong_update_2(self, db_session, server, chat_1, store, preparing_state, wrong_end_game_update):
        """ Checks handling /end update gotten not from a creator while session in preparing_state """
        await store.bots_manager.handle_update(update=wrong_end_game_update)
        assert store.external_api.send_message.call_count == 1
        async with db_session.begin() as session:
            game_session = await store.game_sessions.get_current_session(session, chat_1.id, StatesEnum.PREPARING.value)
        assert game_session