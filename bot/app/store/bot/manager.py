...
class BotManager:
            ...
            for session in sessions:
                if session.state == StatesEnum.WAITING_WORD.value:
                    ...
                    next_player = await self.app.store.game_sessions.get_next_player(
                        db_session, word.proposed_by, session.id
                    )
    ...
    async def on_vote(
        ...
        if session.state == StatesEnum.PREPARING.value:
            await self.send_message(
                update.message.chat_id, MessageHelper.game_is_not_launched
            )
            return
        if session.state == StatesEnum.WAITING_WORD.value:
            await self.send_message(
        ...
        if not session_players:
            await self.send_message(
                update.message.chat_id, MessageHelper.cant_vote(update)
            )
            return
    ...
    async def launch_session(
        self, db_session: AsyncSession, session: GameSession
    ) -> Word:
    ...
    async def start_vote(
        self, db_session: AsyncSession, session: GameSession, word: Word
    ) -> None:
        """Setting the VOTE state of the game session and starting the wait votes timer"""
        await self.app.store.game_sessions.set_session_state(
            db_session, session.id, StatesEnum.VOTE.value
        )
        await self.wait_vote(session, word)

    async def wait_word(
        self, session: GameSession, player: Player, last_word: Word
    ) -> None:
        await self.send_message(
            session.chat_id, MessageHelper.remind_word_for_next(last_word, player)
        )
        timer = AsyncTimer(
            timeout=self.app.config.game.word_wait_time,
    ...
    async def wait_vote(self, session: GameSession, word: Word) -> None:
        ...
        timer = AsyncTimer(
            timeout=self.app.config.game.vote_wait_time,
    ...
    async def drop_player(
        ...
        if len(remaining_players) < 2:
            ...
            await self.send_message(
                session.chat_id, MessageHelper.announce_winner(remaining_player)
            )
            await self.send_message(
                session.chat_id, MessageHelper.game_results(session_players)
            )
            return
    ...
    async def check_vote_results(
        ...
        if len(word_votes) == 0 or len(yes_votes) / len(word_votes) >= 0.5:
            ...
            await self.send_message(
                session.chat_id,
                MessageHelper.vote_result_positive(word_to_vote, player, points),
            )
    ...
    async def check_vote_allowance(
        self,
        db_session: AsyncSession,
        update: Update,
        word_to_vote: Word,
    ...
    async def delete_if_wrong_message(
        self, update: Update, session: GameSession = None
    ) -> Optional[Update]:
        ...
        if len(separate_words) == 1:
            if first in BASIC_COMMANDS or appropriate_state:
                return update
        await self.app.store.external_api.delete_message(
